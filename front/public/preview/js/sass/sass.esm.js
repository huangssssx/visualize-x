...
const init = function () {
  ...
  var SASSJS_RELATIVE_PATH =
    (function () {
      ...
      //web url
      const dirname = scriptUrl.pathname.substring(
        0,
  ...
  function Sass(workerUrl) {
    ...
    that._callbacks = {};
    that._worker = new Worker(workerUrl || globalWorkerUrl);
    that._worker.addEventListener("message", that._handleWorkerMessage, false);
  }
  ...
  Sass.prototype = {
    ...
    destroy: function () {
      that._worker && that._worker.terminate();
      that._worker = null;
      that._callbacks = {};
      that._importer = null;
    },
    ...
    _handleWorkerMessage: function (event) {
      ...
      that._callbacks[event.data.id] &&
        that._callbacks[event.data.id](event.data.result);
      delete that._callbacks[event.data.id];
    },
    ...
    _dispatch: function (options, callback) {
      ...
      that._callbacks[options.id] = callback;
      that._worker.postMessage(options);
    },
    ...
    importer: function (importerCallback, callback) {
      ...
      that._importer = importerCallback;
...
const outsideModule = init()();
debugger;
export default outsideModule;
